...
static TreeNode *parse_tree(string s) {
    ...
    for (int i = 1; i < v.size(); ++i) {
        TreeNode *cur = v[i].find("null") != string::npos
                            ? nullptr
                            : new TreeNode(stoi(v[i]));
        if (i % 2 == 1)
            ns[(i - 1) / 2]->left = cur;
        else
            ns[(i - 1) / 2]->right = cur;
...
static void print(TreeNode *head, int len = 4, int height = 0,
                  string to = "#") {
    if (!head)
        return;
    ...
    val =
        string(height * len, ' ') + string(lenL, ' ') + val + string(lenR, ' ');
    cout << val << endl;
...
static string to_string(ListNode *node) {
    ...
    return to_string(node->val) +
           (node->next ? "->" + to_string(node->next) : "");
}

static void print(ListNode *node) { cout << to_string(node) << endl; }
